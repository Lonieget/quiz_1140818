...
public interface QuizDao extends JpaRepository<Quiz, Integer>{
	...
	@Query(value ="insert into quiz (title, description, start_date, end_date,"//
			+ " publish) values (?1, ?2, ?3, ?4, ?5)" , nativeQuery = true)
	...
	public int seletCountId(int id);
	...
	public List<Quiz> getSearch(String tilte, LocalDate startDate, LocalDate endDate);
	
	
	@Query(value ="select * from quiz where title like %?1% and start_date >= ?2 and"
			+ " end_date <= ?3 and is_publish is true" , nativeQuery = true)
	public List<Quiz> getPublishedSearch(String tilte, LocalDate startDate, LocalDate endDate);
