...
public class QuizService {
	...
	// 提供 json 與 java 物件(Object)之間的互轉
	...
	/**
	 * @Transactional: 1. 使用的於修改資料時(insert/update/dalete) 且有以下2種情形<br>
	 *                 1.1 同一個方法中有使用多個 Dao 時，例如 create 中有同時使用 quizDao 和
	 *                 questionDao<br>
	 *                 1.2 同一個 Dao 有修改多筆的資料，例如下面方法中有使用 questionDao 新增多筆的問題<br>
	 *                 rollbackFor = Exception.class<br>
	 *                 2. 其預設的有效作用是當程式發生 RuntimeException(以及其子類別) 時才會讓資料回朔，所以為了
	 *                 再發生其他 Exception 時也可以讓資料回朔，就要把作用範圍提升到所有例外的父類別：Exception 3.
	 *                 要讓 @Transactional 有效的另一個條件必須要把發生的 Exception 給它 throw 出去
	 */
	...
	public BasicRes create(Quiz quiz, List<QuestionVo> questionVoList) throws Exception {
		try {
			...
			// 取 quiz_id，因為 quiz_id 是流水號，要等資料新增近 DB後才會自動產生
			...
			for (QuestionVo vo : questionVoList) {
				...
				// 將 QuestionVo 中的 List<Optings> 轉成字串
				String optionsStr = mapper.writeValueAsString(vo.getOptionsList());
				// 新增 question
				// 確保這些問題都同一個 quizId --> 使用 quiz 中取得的 id
	...
	private BasicRes checkQuestion(List<QuestionVo> questionVoList) {
		for (QuestionVo item : questionVoList) {
			...
			// 檢查選項 :
			// 正確的應該是: type 是 1. 是簡答題時，不會有選項:或 2. 選擇題(單，多選)時要有選項
	...
	private BasicRes checkDate(LocalDate startDate, LocalDate endDate) {
		// 正常來說，問卷的開始時間一定是等於或晚於結束時間，若 startDate.isAfter(endDate)
	...
	public BasicRes update(Quiz quiz, List<QuestionVo> questionVoList) throws Exception {
		try {
			...
			// 搜尋欄位 id 出現的次數，因為 id 是 PK， 所以結果只會是 0 or 1
			if (quizDao.seletCountId(quizId) != 1) { // 0 表示該 quizId 不存在
	...
	public QuizListRes getQuizList(String title, LocalDate startDate, LocalDate endDate ,boolean getPublish) {
		...
		if (startDate != null && endDate != null && startDate.isAfter(endDate)) {
			return new QuizListRes(ResCodeMessage.QUIZ_ID_ERROR.getCode(),
