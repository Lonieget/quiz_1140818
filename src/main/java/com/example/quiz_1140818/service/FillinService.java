...
public class FillinService {
				...
				try {
					for (Answer item : answerList) {
						fillinDao.fillin(quizId, item.getQuestionId(), user.getEmail(), user.getName(), user.getPhone(),
								user.getAge(), user.getGender(), mapper.writeValueAsString(item), LocalDate.now());
					}
				} catch (Exception e) {
					throw e;
				}
	...
	private BasicRes checkRequiredAnswer(int questionId, List<Answer> answerList, String type) {
		for (Answer answer : answerList) {
			...
			if (answer.getQuestionId() == questionId) {
				...
				} else if (type.equalsIgnoreCase(QuestionType.SINGLE.getType())) {
	...
	public FeedbackRes feeback(int quizId) throws Exception {
		if (quizId <= 0) {
		...
		List<QuestionAnswerVo> questionAnsVoList = new ArrayList<>();
		for (Question question : questionList) {
		...
		// 把 Fillin 的答案放到 QuestionAnswerVo 中
		...
		for (Fillin item : fillinList) {
			...
			if (!emailFeedbackVoMap.containsKey(email)) {// 表示尚未找廖該 user 的答案
	...
	// 將 Question 的所有選擇題的基本資料(不包含選項和次數)
